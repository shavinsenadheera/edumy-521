<?php
	...
	use Modules\Course\Models\CourseDate;
	...
	class AvailabilityController extends FrontendController{
		...
		protected $indexView = 'Course::frontend.user.availability';
		...
		public function index(Request $request){
			...
			$page_title = __('Courses Availability');
		...
		public function loadDates(Request $request){
			...
			$course = $this->courseClass::find($request->query('id'));
			if(empty($course)){
				return $this->sendError(__('Course not found'));
			}
			$is_single = $request->query('for_single');

			$query = $this->courseDateClass::query();
			...
			for($i = strtotime($request->query('start')); $i <= strtotime($request->query('end')); $i+= DAY_IN_SECONDS)
			{
				$date = [
					'id'=>rand(0,999),
					'active'=>0,
					'price'=>(!empty($course->sale_price) and $course->sale_price > 0 and $course->sale_price < $course->price) ? $course->sale_price : $course->price,
					'is_default'=>true,
					'textColor'=>'#2791fe'
				];
				...
				if($request->input('for_single')){
					if(empty(!$course->max_people) and $course->max_people < 1){
						$date['active'] = 0;
					}
				}

				if(!empty($course->meta->enable_person_types) and $course->meta->enable_person_types==1){
					$date['person_types'] = $course->meta->person_types ;
				...
				// Open Hours
				if (!empty($course->meta->enable_open_hours) and $course->meta->enable_open_hours == 1) {
					$open_hours = $course->meta->open_hours;
					$nDate = date('N', $i);
			...
			if(!empty($rows))
			{
				foreach ($rows as $row)
				{
					...
					if(!empty($course->meta->enable_person_types) and $course->meta->enable_person_types==1){
						$list_person_types =$course->meta->person_types;
			...
			$bookings = $this->bookingClass::getBookingInRanges($course->id,$course->type,$request->query('start'),$request->query('end'));
			if(!empty($bookings))
			{
		...
		public function store(Request $request){
			...
			$course = $this->courseClass::find($request->input('target_id'));
			...
			{
				$date = $this->courseDateClass::where('start_date',date('Y-m-d',$i))->where('target_id',$target_id)->first();
				if(empty($date)){
					$date = new $this->courseDateClass();
