        ...
        foreach($setting as $l=>$menuId){
            if($l == $location and $menuId){
                ...
                if(!empty($translation)){
                    if(!empty($options['device']) && $options['device'] == 'mobile'){
                        $walker->generateMobile();
                    }else{
                        $walker->generate();
                    }
...
function get_image_tag($image_id,$size = 'thumb',$options = []){
    ...
    if($url){
        ...
        if(!empty($options['lazy'])){
            $class.=' lazy';
            $class.=' lazy  ';
            $attr.=" src=".$nullSrc." data-src=".e($url)." ";
        }else{
            $attr.=" src=".e($url)." ";
        }
...
function convertFloatToHour($time){
    return sprintf('%02d:%02d', (int) $time, fmod($time, 1) * 60);
}

function convertToHoursMinutes($time, $format = '%02d:%02d') {
    if ($time < 1) {
        return $time;
    }
    $hours = floor($time / 60);
...
function get_cart_fragments(){
    return [
        '.cart_btn .dropdown_content'=>view('Booking::frontend.cart.mini-cart')->render(),
        '.cart_btn .cart_btn  span'=>Cart::count()
    ];
}
